...
if [ $ledger = true ]; then
    forge script -l -f "$rpcUrl" script/01_DeployProtocol.s.sol:Deploy --broadcast --slow
else
    forge script --private-key "$deployerPrivateKey" -f "$rpcUrl" script/01_DeployProtocol.s.sol:Deploy --broadcast --slow
fi
