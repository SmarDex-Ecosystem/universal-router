...
library SmardexLibrary {
    ...
    function getUpdatedPriceAverage(
        ...
        // need to compute new linear-average price
            ...
            newPriceAverageIn_ = _fictiveReserveIn;
            newPriceAverageOut_ = (
                ((MAX_BLOCK_DIFF_SECONDS - _timeDiff) * _priceAverageOut * newPriceAverageIn_) / _priceAverageIn
                    + _timeDiff * _fictiveReserveOut
    ...
    function computeFirstTradeQtyIn(GetAmountParameters memory _param) internal pure returns (uint256 firstAmountIn_) {
        ...
        if (_param.fictiveReserveOut * _param.priceAverageIn > _param.fictiveReserveIn * _param.priceAverageOut) {
            ...
            uint256 _inSqrt = (((_param.fictiveReserveIn * _param.fictiveReserveOut) * 4) / _param.priceAverageOut)
                * _param.priceAverageIn * ((FEES_BASE - _param.feesPool - _param.feesLP) * (FEES_BASE - _param.feesPool))
     ...
     */
    function computeFirstTradeQtyOut(GetAmountParameters memory _param)
        internal
        pure
        returns (uint256 firstAmountOut_)
    {
        // default value
        ...
        if (_param.fictiveReserveOut * _param.priceAverageIn > _param.fictiveReserveIn * _param.priceAverageOut) {
            // pre-compute all operands
            uint256 _fictiveReserveOutPredFees =
                (_param.fictiveReserveIn * _param.feesLP * _param.priceAverageOut) / _param.priceAverageIn;
            uint256 _toAdd = ((_param.fictiveReserveOut * _reverseFeesTotal) * 2) + _fictiveReserveOutPredFees;
            uint256 _toDiv = _reverseFeesTotal * 2;

            uint256 _inSqrt = (
                ((_param.fictiveReserveOut * _fictiveReserveOutPredFees) * 4)
                    * (_reverseFeesTotal * (FEES_BASE - _param.feesPool))
            ) / _param.feesLP + (_fictiveReserveOutPredFees * _fictiveReserveOutPredFees);
    ...
    function computeFictiveReserves(
        ...
        if (_reserveOut * _fictiveReserveIn < _reserveIn * _fictiveReserveOut) {
            ...
            newFictiveReserveIn_ = (_temp * _fictiveReserveIn) / _fictiveReserveOut
                + (_reserveOut * _fictiveReserveIn) / _fictiveReserveOut;
     ...
     */
    function applyKConstRuleOut(GetAmountParameters memory _param)
        internal
        pure
        returns (
            uint256 amountOut_,
     ...
     * @return newFictiveReserveOut_ new fictive reserve of the out-token after the transaction
     */
    function applyKConstRuleIn(GetAmountParameters memory _param)
        internal
        pure
        returns (
            uint256 amountIn_,
        ...
        )
    {
        ...
        uint256 _denominator =
            (_param.fictiveReserveOut - _param.amount) * (FEES_BASE - _param.feesPool - _param.feesLP);
     ...
     */
    function getAmountOut(GetAmountParameters memory _param)
        internal
        pure
        returns (
            uint256 amountOut_,
        ...
        )
    {
        ...
        require(
            _param.reserveIn != 0 && _param.reserveOut != 0 && _param.fictiveReserveIn != 0
        ...
        if (
            _firstAmountIn == _amountInWithFees
                && ratioApproxEq(
                    _param.fictiveReserveIn, _param.fictiveReserveOut, _param.priceAverageIn, _param.priceAverageOut
                )
        ) {
            (_param.fictiveReserveIn, _param.fictiveReserveOut) = computeFictiveReserves(
                _param.reserveIn, _param.reserveOut, _param.fictiveReserveIn, _param.fictiveReserveOut
            );
        }
        ...
        if (_firstAmountIn < _amountInWithFees) {
            ...
            (newFictiveReserveIn_, newFictiveReserveOut_) =
                computeFictiveReserves(newReserveIn_, newReserveOut_, newFictiveReserveIn_, newFictiveReserveOut_);

            uint256 _secondAmountOutNoFees;
            (_secondAmountOutNoFees, newReserveIn_, newReserveOut_, newFictiveReserveIn_, newFictiveReserveOut_) =
            applyKConstRuleOut(
                SmardexLibrary.GetAmountParameters({
     ...
     * @return newFictiveReserveOut_ fictive reserve of the buying token after the swap
     */
    function getAmountIn(GetAmountParameters memory _param)
        internal
        pure
        returns (
            uint256 amountIn_,
        ...
        )
    {
        ...
        require(
            _param.amount < _param.fictiveReserveOut && _param.reserveIn != 0 && _param.reserveOut != 0
                && _param.fictiveReserveIn != 0 && _param.fictiveReserveOut != 0,
            "SmarDexLibrary: INSUFFICIENT_LIQUIDITY"
        );
        ...
        if (_firstAmountOut < _param.amount) {
            ...
            (_secondAmountIn, newReserveIn_, newReserveOut_, newFictiveReserveIn_, newFictiveReserveOut_) =
            applyKConstRuleIn(
                SmardexLibrary.GetAmountParameters({
